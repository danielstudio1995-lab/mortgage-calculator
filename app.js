...
const formatInput=el=>{const v=parseNum(el.value);if(v>=0)el.value=v>0?v.toLocaleString('he-IL'):'0';};
...
/* ── MATH ── */
function spitzer(P,r,y){if(P<=0||r<=0||y<=0)return 0;const rm=r/100/12,n=y*12;return P*rm*Math.pow(1+rm,n)/(Math.pow(1+rm,n)-1);}
...
function gracePayments(P,r,y,gMo){const gp=P*(r/100/12);const postY=Math.max((y*12-gMo)/12,.1);return{grace:gp,post:spitzer(P,r,postY)};}
...
function computeTrack(id,t,totalM,cpi){
  ...
  if(t.amort==='keren'){initial=kerenInitial(P,rate,years);peak=initial;total=kerenTotal(P,rate,years);}
  else if(t.amort==='balloon'){initial=balloonMonthly(P,rate);peak=balloonMonthly(P,rate+2);total=initial*balloonMo+P;balloonEnd=P;}
  else if(t.amort==='grace'){const g=gracePayments(P,rate,years,graceMo);initial=g.grace;peak=g.post;total=g.grace*graceMo+g.post*(years*12-graceMo);}
  else{
    if(t.cpi){const eff=rate+cpi;initial=spitzer(P,eff,years);const gP=P*Math.pow(1+cpi/100,Math.min(10,years));peak=spitzer(gP,eff,Math.max(years-10,1));total=spitzerTotal(P,eff,years)*Math.pow(1+cpi/100,years*.5);}
    else if(!t.fixed){initial=spitzer(P,rate,years);peak=spitzer(P,rate+2,years);total=spitzerTotal(P,rate,years);}
    else{initial=spitzer(P,rate,years);peak=initial;total=spitzerTotal(P,rate,years);}
  }
...
const TRACK_INFO={
  kalatz:{title:'קבועה לא צמודה (קל"צ)',body:'ריבית קבועה לכל אורך חיי ההלוואה, ללא הצמדה. ההחזר קבוע ויציב לחלוטין. מסלול הביטחון המרכזי.'},
  katz:  {title:'קבועה צמודת מדד (ק"צ)',body:'ריבית קבועה, אך הקרן צמודה למדד. ריבית נמינלית נמוכה יותר, אך ההחזר גדל עם האינפלציה.'},
  ...
  prime: {title:'פריים',body:'ריבית = ריבית בנק ישראל + 1.5%. גמיש לפירעון מוקדם. חשוף לשינויי ריבית בנק ישראל.'},
...
function updateStep1(){
  ...
  if(purchaseType==='discounted'){
    ...
    equityPct=propertyPrice>0?(reqEquity/propertyPrice*100):25;
    ...
    if(equityHint)equityHint.textContent=`מינימום הון עצמי: ${Math.ceil(minEquity).toLocaleString('he-IL')} ₪`;
    const priceHint=document.getElementById('priceHint');if(priceHint)priceHint.textContent='';
    if(ltvPct>maxLTV){
    ...
    const equityHint=document.getElementById('equityHint');if(equityHint)equityHint.textContent='';
  ...
  // Monthly range (25 years, 2.8%–5.2%)
  ...
  if(valid&&mortgage>0){
    ...
    if(mrBox){
      mrBox.classList.add('visible');
      setText('mrMin',fmt(spitzer(mortgage,2.8,25)));
      setText('mrMax',fmt(spitzer(mortgage,5.2,25)));
    }
...
function syncTotalMortgageField(){
  ...
  // Update all amt displays
  Object.entries(STATE.tracks).forEach(([id,t])=>{
    if(t.pct>0&&t.amt===0){t.amt=Math.round(STATE.totalMortgage*(t.pct/100));}
    syncAmtDisplay(id);
  });
  recalcAllTracks();
}
...
/* ── AMT SYNC ── */
function syncAmtDisplay(id){
  const el=document.getElementById(`${id}AmtNum`);
  const sld=document.getElementById(`${id}PctSlider`);
  const t=STATE.tracks[id];
  if(!t)return;
  if(el)el.value=t.amt>0?t.amt.toLocaleString('he-IL'):'0';
  // Compute pct from amt
  if(STATE.totalMortgage>0){t.pct=Math.round(t.amt/STATE.totalMortgage*100);}
  if(sld){sld.value=t.pct;setSliderFill(sld);}
}

function onAmtChange(id,amt){
  const t=STATE.tracks[id];if(!t)return;
  t.amt=amt;
  if(STATE.totalMortgage>0)t.pct=Math.round(amt/STATE.totalMortgage*100);
  const sld=document.getElementById(`${id}PctSlider`);
  if(sld){sld.value=t.pct;setSliderFill(sld);}
  recalcAllTracks();
}
...
function onSliderChange(id,pct){
  const t=STATE.tracks[id];if(!t)return;
  t.pct=pct;
  t.amt=Math.round(STATE.totalMortgage*(pct/100));
  const el=document.getElementById(`${id}AmtNum`);
  if(el)el.value=t.amt>0?t.amt.toLocaleString('he-IL'):'0';
  recalcAllTracks();
}
...
function recalcAllTracks(){
  ...
  let fixedAmt=0,allocatedAmt=0,activeCount=0;
  const breakdowns=[];
  ...
  Object.entries(STATE.tracks).forEach(([id,t],i)=>{
    ...
    activeCount++;allocatedAmt+=res.P;
    ...
    const beRow=document.getElementById(`${id}-balloon-end`);
    const beVal=document.getElementById(`${id}BalloonEnd`);
  ...
  // Fixed rule (by amount)
  const fixedPct=totalM>0?Math.round(fixedAmt/totalM*100):0;
  const fixedOk=fixedAmt>=totalM/3;
  setText('fixedPct',fixedPct+'%');
  ...
  if(pctMsg){
    const remain=totalM-allocatedAmt;
    const ok=Math.abs(remain)<1000;
  ...
  updateStickySummary(allocatedAmt,totalMonthly,breakdowns);
}
...
function renderBreakdown(items){
  ...
  items.forEach(item=>{
    ...
    seg.style.cssText=`flex:${item.amt};background:${item.color};`;
    bar.appendChild(seg);
  });
}
...
function updateStickySummary(allocated,monthly,breakdowns){
  ...
  setText('ssTotalAmt',fmt(STATE.totalMortgage));
  setText('ssMonthly',monthly>0?fmt(monthly):'—');

  const remain=STATE.totalMortgage-allocated;
  const remainEl=document.getElementById('ssRemain');
  if(remainEl){
    remainEl.textContent=Math.abs(remain)<1000?'הוקצה הכל':fmt(Math.abs(remain));
    remainEl.className='ss-val'+(remain<-1000?' over':'');
  }
  ...
  if(tracksEl){
    ...
    breakdowns.forEach(b=>{
      const chip=document.createElement('div');chip.className='ss-track-chip';
      chip.innerHTML=`<span class="ss-track-name" style="color:${b.color}">${TRACK_LABELS[b.id]||b.id}</span><span class="ss-track-amt">${fmt(b.amt)}</span>`;
      tracksEl.appendChild(chip);
    });
  }
}
...
function renderCompareTable(){
  ...
  html+='</tr></thead><tbody>';
  const rows=[['סכום משכנתא','totalMortgage',fmt],['החזר חודשי','totalMonthly',fmt,'bestMonthly'],['החזר חודשי בשיא','totalPeak',fmt],['עלות ריבית','totalInterest',fmt],['סך החזר','grandTotal',fmt,'bestTotal']];
  rows.forEach(([label,key,fn,bk])=>{
    html+=`<tr><td><strong>${label}</strong></td>`;
    mixes.forEach(m=>{const v=m.data[key],isBest=bk&&((bk==='bestMonthly'&&v===bestMonthly)||(bk==='bestTotal'&&v===bestTotal));html+=`<td class="${isBest?'best':''}">${fn(v)}${isBest?' ⭐':''}</td>`;});
    html+='</tr>';
  });
  const allIds=[...new Set(mixes.flatMap(m=>m.data.tracks.map(t=>t.id)))];
  allIds.forEach(tid=>{
    html+=`<tr><td><em>${TRACK_LABELS[tid]||tid}</em></td>`;
    mixes.forEach(m=>{const t=m.data.tracks.find(t=>t.id===tid);html+=`<td>${t?`${fmt(t.amt)} | ${t.rate.toFixed(2)}% | ${Math.round(t.years)}שנ`:'—'}</td>`;});
    html+='</tr>';
  });
  html+='</tbody></table>';wrap.innerHTML=html;
  wrap.querySelectorAll('.delete-mix-btn').forEach(btn=>{
    btn.addEventListener('click',()=>{STATE.savedMixes=STATE.savedMixes.filter(m=>m.id!==parseInt(btn.dataset.mixId));renderCompareTable();});
  });
}
...
document.addEventListener('DOMContentLoaded',()=>{
  ...
  document.querySelectorAll('.tab').forEach(tab=>{
    tab.addEventListener('click',()=>{
      ...
      else{document.getElementById('stickySummary')?.classList.remove('visible');document.body.classList.remove('has-sticky');}
    });
  });
  ...
  document.querySelectorAll('.purchase-card').forEach(card=>{
    card.addEventListener('click',()=>{
      ...
      if(df)df.classList.toggle('visible',isDisc);
      if(s2)s2.style.display=isDisc?'none':'block';
      updateStep1();
    });
  });
  ...
  document.getElementById('hasGrant')?.addEventListener('change',e=>{
    STATE.hasGrant=e.target.checked;updateStep1();
  });
  ...
  ['Fixed','Var'].forEach(g=>{
    ...
    if(btn&&body&&arrow){
      btn.addEventListener('click',()=>{const c=body.classList.toggle('collapsed');arrow.classList.toggle('closed',c);});
    }
  });
  ...
  document.querySelectorAll('.track-card').forEach(card=>{
    ...
    if(amtEl){
      amtEl.addEventListener('input',()=>onAmtChange(id,parseNum(amtEl.value)));
      amtEl.addEventListener('blur',()=>formatInput(amtEl));
    }
    ...
    if(rateSld&&id!=='zakaut'){
      rateSld.addEventListener('input',()=>{t.rate=parseInt(rateSld.value)/100;setText(`${id}Rate`,t.rate.toFixed(2)+'%');setSliderFill(rateSld);recalcAllTracks();});
